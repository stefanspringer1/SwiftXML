...
With regard to namespaces, the treatment of attributes corresponds to the treatment of elements, and independently of each other. The attributes that do not have an explicit namespace prefix set in the source do not get a namespace prefix during parsing, regardless of whether the corresponding element belongs to a namespace. So e.g. `myElement["id"]` also returns the according attribute value of an element that has a prefix, and `document.registeredAttributes("id")` finds all these attributes if `"id"` is a registered attribute name. Consequently, changing the prefix of an element does not change the prefixes of its attributes. Set or get an attribute value via `element[prefix,name]`, where `prefix` might be `nil`. 
