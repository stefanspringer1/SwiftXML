...
This iteration though the elements of a document by name does not have to search for those elements, but elements (and attributes as far as they are registered) can be accessed directly by their names. The order of such an iteration is the order by which the item is added to the document.
