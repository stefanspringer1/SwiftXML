...
The namspaces and their prefixes are registerd at the document, according attributes are not (!) set and only only appear in a serialization of the document or parts of it (they appear after every other attribute at the root element of the serialization). To regisster a new namespace with its prefix, use the method `XDocument.register(namespaceURI:,withPrefixSuggestion:)` which returns the actual prefix being used.
