...
For a single node, use the `checking(...)` method to see if a condition is met; if yes, the node is returned, if not, `nil` is returned. Use `hasProperty(...)` to see if a node has a certain property.
