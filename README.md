...
The namespaces with their prefixes are registerd at the document, according attributes are not (!) set in the tree and only appear in a serialization of the document or parts of it (they appear after every other attribute at the top element of the serialization). To register a new namespace with its prefix, use the method `XDocument.register(namespaceURI:,withPrefixSuggestion:)` which returns the actual prefix being used.
