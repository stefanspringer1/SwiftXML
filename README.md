...
Some XML documents declare a namespace at the top of the document in the form `xmlns="..."` i.e. without a prefix to define the schema to be used for the document. When reading such a document with `recognizeNamespaces: true`, a prefix is created for this namespace and used for all according elements, consequently rules and the name based searches made available by this libary have to take that prefix into account via the argument `prefix:`. If want to avoid this, use the setting `noPrefixForPrefixlessNamespaceAtRoot: true` when parsing. The according namespace URI is then still registered at the document, but with prefix value `""`, the according elements then have no prefix value set (their prefix value is `null`), so e.g. no prefix value has to be considered in rules. However, it is then no longer possible to distinguish between elements with this or another namespace on the one hand and elements without a namespace on the other.
