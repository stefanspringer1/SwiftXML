...
In the code of this transformation you see an `XElement` being initialized at some point. The `SwiftXML` package uses names not starting with `XML`, but names starting with just `X` instead. Names starting with `XML` belong to the `FoundationXML` implementation which is part of the Swift toolchain but which is not used here. The subscript notation `element["..."]` of an element is used to get or set an attribute value.
