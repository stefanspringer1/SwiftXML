...
When writing code that takes a possible prefix into account (i.e., the code should work regardless of whether `prefix` has a value or is `nil`), always test your code with appropriate prefixes set, e.g., with the default `silentEmptyRootPrefix: false` and an according namespace definition in the source.
